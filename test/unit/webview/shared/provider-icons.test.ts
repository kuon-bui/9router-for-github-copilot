...
describe('resolveProviderIcon', () => {
  it.each([
    ['antigravity', 'antigravity'], ['claude', 'claude'], ['codebuddy-cn', 'codebuddy'],
    ['codebuddy-intl', 'codebuddy'], ['codex', 'codex'], ['deepseek', 'deepseek'],
    ['gemini-cli', 'gemini'], ['github', 'copilot'], ['glm', 'zai'], ['glm-cn', 'zhipu'],
    ['grok-cli', 'grok'], ['kimi', 'kimi'], ['kiro', 'kiro'], ['minimax', 'minimax'],
    ['minimax-cn', 'minimax'], ['ollama', 'ollama'], ['qoder', 'qoder'],
    ...
    expect(icon?.url).toMatch(new RegExp(
      `^https://unpkg\\.com/@lobehub/icons-static-svg@latest/icons/${expectedSlug}(?:-color)?\\.svg$`
    ));
  });

  it.each([
    ['cx', 'codex'], ['cbcn', 'codebuddy'], ['cbai', 'codebuddy'], ['cc', 'claude'],
    ['gc', 'gemini'], ['gh', 'copilot'], ['grok-build', 'grok'], ['kimi-coding', 'kimi'],
    ['vercel', 'vercel'], ['xai', 'xai'], ['openai', 'openai'], ['trae', 'trae']
