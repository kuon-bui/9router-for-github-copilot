...
export function ModelEditor({ api }: { api: VsCodeApi }): JSX.Element {
  ...
  if (editing === undefined) return <ModelList state={state} error={error} onAdd={() => { setError(''); setEditing(null); }} onEdit={(sourceIndex) => { setError(''); setEditing(sourceIndex); }} onRemove={(sourceIndex) => api.postMessage({ type: 'removeModel', sourceIndex })} onMove={(sourceIndex, direction) => api.postMessage({ type: 'moveModel', sourceIndex, direction })} onRefreshCatalog={() => api.postMessage({ type: 'refreshCatalog' })} />;
